...
// Initialize Supabase after fetching config from the server
async function initSupabase() {
    try {
        const response = await fetch('/api/config');
        const config = await response.json();
        
        // Initialize Supabase with the config from the server
...
function showView(viewName) {
    ...
    document.querySelectorAll('[id$="-view"]').forEach(view => {
        ...
        view.classList.remove('fade-in', 'slide-up');
    });
...
async function initializeDatabase() {
    try {
        ...
        if (deptError && deptError.code === 'PGRST116') { // Table doesn't exist
            // Create departments table and insert data
            const { error: createDeptError } = await supabase.rpc('create_departments_table');
            if (createDeptError) {
                console.error('Error creating departments table:', createDeptError);
            }
        }
        ...
        if (profilesError && profilesError.code === 'PGRST116') { // Table doesn't exist
